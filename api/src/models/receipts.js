...
class Receipts {
  ...
  async getReceiptsByGroupId(groupId) {
    this.connectDAO();
    const response = await this.dao.all(
      `SELECT * FROM receipts WHERE groupId = ?`,
      [groupId]
    );
  ...
  async delete(id, userId) {
    this.connectDAO();
    const response = await this.dao.run(
      `DELETE FROM receipts WHERE id = ? AND ownerId = ?`,
      [id, userId]
    );
    this.dao.close();
    return response;
  ...
  async personPay(id, userId, indexList) {
    var rec = await this.dao.run(`SELECT receipt FROM receipts WHERE id = ?`, [
      id
    ]);
